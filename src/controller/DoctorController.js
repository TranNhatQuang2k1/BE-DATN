...
let getRevenueOfAllDoctors = async (req,res) => {
    if(!req.query.begin){
        return res.status(400).json({
            errCode:"2",
    ...
    let resData = await doctorService.getRevenueOfAllDoctors(data);
    let page ={}
    page.size= resData.size
    page.totalPages= resData.totalPages
    page.totalElements = resData.totalElements
...
let getRevenueOfDoctor =async (req,res) => {
    if(!req.query.begin){
        return res.status(400).json({
            errCode:"2",
            message: 'thiếu thời gian bắt đầu'
    ...
    let resData = await doctorService.getRevenueOfDoctor(data);
